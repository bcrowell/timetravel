...
def main()
  ...
  if pass>=2 then
    if pass<=$freeze_at_pass then 
      ...
      $aux_invoked,$aux_par,$aux_landed = remember_page_numbers()
    end
  end
  ...
  File.readlines(in_file).each { |line|
    ...
    if line_type=='end' then
      ...
      if pass==1 then
        ...
        File.open(code_file,'w') { |code_f| code_f.print "\\timetraveldisable{}#{code}\\label{timetravellanded#{key}}\\timetravelenable{}" }
        code = ''
      end
      if pass>=2 then
...
def save_page_numbers
  ...
  File.open("#{$temp_dir}/freeze_aux_landed.json",'w') { |f|
    f.print JSON.generate($aux_landed)
  }
end
...
def remember_page_numbers
  return [
    ...
    get_json_data_from_file_or_die("#{$temp_dir}/freeze_aux_landed.json")
  ]
end
